 ...
 * your attention, so it isn't counted. Refreshes on auth change, whenever
 * refresh() is called explicitly (TradesComponent calls it after loading/
 * resolving offers), and now also on EventsService's lastTradeUpdate — a
 * new offer, or one of your own being accepted/declined/cancelled, updates
 * this badge live in any open tab instead of only after a manual refresh.
 */
...
export class TradesNotificationService {
  ...
  constructor() {
    ...
    effect(() => {
      if (this.events.lastTradeUpdate() && this.auth.accessToken()) {
        this.refresh();
      }
    });
  }
