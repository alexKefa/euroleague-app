  ...
  template: `
    <svg viewBox="0 -90 320 300" class="w-full h-full pointer-events-none" preserveAspectRatio="xMidYMid meet">
      ...
      <!-- Center-court logo decal — the app's own standalone icon (same
           path data as favicon-v5.svg: a thin "C" ring with the ball
           dead-centered inside it, padded clear of the ring on every
           side), faint and fixed-color like the rest of this glass floor
           rather than theme-reactive, painted over the floor but under
           the real court lines so the key/arc strokes stay crisp on top
           of it. Centered offset is a plain translate(-50 -50) now — the
           mark itself is symmetric around (50,50), unlike the old
           arc+line+ball icon this replaced, so no hand-tuned recentering
           is needed. -->
      <g transform="translate(160 63) scale(1.7) translate(-50 -50)" opacity="0.16">
        ...
        <path d="M 74.2 29 A 32 32 0 1 0 74.2 71" fill="none" stroke="#eef3f7" stroke-width="7" stroke-linecap="round" />
        <circle cx="50" cy="50" r="21" fill="url(#ballGradCourt)" />
